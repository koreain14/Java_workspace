...
public class AppMain extends JFrame {
	private static Connection con; // 공유, get메서드로 접근 
	...
	public void connect(){
		// 자바 소스코드 외부에 둔 특정파일의 내용을 읽어와서 데이터베이스 정보를 구성하면
		// 유지보수성이 높아진다.
		// 왜?? DB정보가 변경되어도 자바소스를 바꾸지 않아도 된다!
		...
		FileInputStream fis=null;
			...
			try {
				...
				// 데이터베이스 접속 처리
				con=DriverManager.getConnection(host, user, pwd); // con으로 받아라!! 접속정보 포함!!
				if(con!=null){
					setTitle("접속성공");
				}else{
					setTitle("실패");
	...
	// 외부의 객체들은 이 메서드를 통해서만 공유된 Connection을 가져가게 하자
